...
<script language="JavaScript"> document.location.replace("<%=lien%>?but=<%=bute%>&id=<%=idmere%>");</script>
<% }
    if(acte.compareToIgnoreCase("updateEchange")==0){
        mere = (ClassMAPTable) (Class.forName(classe).newInstance());
        fille = (ClassMAPTable) (Class.forName(classefille).newInstance());

        PageUpdateMultiple p =new PageUpdateMultiple(mere,fille,request,nbLine,tId);
        Vente vmere = (Vente) p.getObjectAvecValeur();
        ClassMAPTable[] vfille = p.getObjectFilleAvecValeur();
        for (ClassMAPTable details:vfille){
            details.setNomTable("Vente_details");
        }
        vmere.controlleEchange(vfille,null);
        ClassMAPTable o = (ClassMAPTable) u.updateObjectMultiple(vmere, colonneMere, vfille);
        temp = (Object) o;
        if (temp != null) {
            val = temp.toString();
            idmere = o.getTuppleID();
    ...
    if(acte.compareToIgnoreCase("updateEchangePayer")==0){
        mere = (ClassMAPTable) (Class.forName(classe).newInstance());
        fille = (ClassMAPTable) (Class.forName(classefille).newInstance());
        ...
        vmere.controlleEchange(vfille,null);
        ClassMAPTable o = (ClassMAPTable) u.updateObjectMultiple(vmere, colonneMere, vfille);
        MvtCaisse mvt = vmere.encaisser(u);
        temp = (Object) o;
        if (temp != null) {
            val = temp.toString();
            idmere = o.getTuppleID();
...
<%
    ...
    ex.printStackTrace();
%>
<script type="text/javascript">alert("<%=ex.getMessage()%>");
history.back();</script>
<%
        return;
    }%>
