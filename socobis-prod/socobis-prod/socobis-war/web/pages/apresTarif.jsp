...
<%@ page import="vente.VenteDetailsCpl" %>
...
<%
    ...
    if (acte.compareToIgnoreCase("update_payer") == 0) {
        t = (ClassMAPTable) (Class.forName(classe).newInstance());
        Page p = new Page(t, request);
        VenteDetails f = (VenteDetails) p.getObjectAvecValeur();
        temp = f;
        if (nomtable != null) {
            f.setNomTable(nomtable);
        }
        ...
        VenteDetails vD = (VenteDetails) CGenUtil.rechercher(new VenteDetails(), null, null, null, " and ID='" + f.getId() + "' ")[0];

        VenteDetails updated = (VenteDetails) u.updateObject(f);


        Vente mere = (Vente)  vD.findMere("VENTE",null);

        MvtCaisse mvt = mere.encaisser(u);

//        double diff = updated.calculerTTC()-vD.calculerTTC();
//        if(diff>0){
//            ComptaEcriture ecriture = (ComptaEcriture) CGenUtil.rechercher(new ComptaEcriture(),null,null,null," AND idObjet='"+mere.getId()+"'")[0];
//            ComptaSousEcriture sousEcriture = new ComptaSousEcriture();
//            sousEcriture.setIdMere(ecriture.getId());
//            sousEcriture.setExercice(Integer.parseInt(ecriture.getExercice()));
//            sousEcriture.setDaty(mere.getDaty());
//            sousEcriture.setJournal(ConstanteStation.JOURNALVENTE);
//            sousEcriture.setCredit(diff);
//            sousEcriture.setDebit(diff);
//
//            if(sousEcriture.getDebit()>0 || sousEcriture.getCredit()>0) sousEcriture.createObject(u.getUser().getTuppleID(), null);
//            sousEcriture.validerObject(u.getUser().getTuppleID(),null);
//        }
